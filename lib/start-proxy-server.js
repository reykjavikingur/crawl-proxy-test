var _ = require('underscore');
var express = require('express');
var bodyParser = require('body-parser');
var request = require('request');
var scrubber = require('./scrubber');
var liberateCookie = require('./liberate-cookie');

module.exports = startProxyServer;

function startProxyServer(options) {

	var app = express();

	app.use(bodyParser.raw({
		type: '*/*'
	}));

	app.all('*', function(req, res, next) {
		var host = req.headers.host;
		var proxyTarget = options.proxyTarget;

		// construct request to proxy target
		var proxyReq = {
			method: req.method,
			url: options.proxyTarget + req.url,
			followRedirect: false,
			headers: _.clone(req.headers)
		};
		// scrub headers that contain proxy URL
		var scrub = scrubber(host, proxyTarget);
		_.each(['host', 'origin', 'referer'], function(field) {
			if (proxyReq.headers[field]) {
				proxyReq.headers[field] = scrub(proxyReq.headers[field]);
			}
		});
		// relay request contents
		if (['POST', 'PUT', 'PATCH'].indexOf(req.method) >= 0 && req.body) {
			proxyReq.body = req.body;
		}
		// send request to proxy target
		request(proxyReq, function(err, proxyRes, body) {
			if (err) {
				console.error('Request Error:', err);
				res.status(500).send('proxy error');
			} else {
				if (proxyRes.headers['set-cookie']) {
					proxyRes.headers['set-cookie'] = liberateCookie(proxyRes.headers['set-cookie']);
				}
				var scrub = scrubber(proxyTarget, host);
				_.each(['location', 'refresh'], function(field) {
					if (proxyRes.headers[field]) {
						proxyRes.headers[field] = scrub(proxyRes.headers[field]);
					}
				});
				res.set(proxyRes.headers);
				res.status(proxyRes.statusCode);
				res.send(body);
			}
		});

	});

	app.listen(options.port, function() {
		console.log('proxy server listening on port ' + options.port);
	});

}